...
const previewFactories = Object.freeze({
  "app-shell": ({ copy }) => `
    <div class="vp-shell">
      <div class="vp-shell-top"><span class="vp-logo"></span>${line("short")}<span class="vp-avatar"></span></div>
      <div class="vp-shell-body"><div class="vp-sidebar"><span class="is-active"></span><span></span><span></span><span></span></div><div class="vp-workspace"><small>${copy.dashboard}</small>${line("title")}<div class="vp-metrics"><i></i><i></i><i></i></div><div class="vp-chart"></div></div></div>
    </div>`,
  header: ({ copy }) => `
  ...
  responsive: () => `
    <div class="vp-responsive"><div class="vp-device vp-device--desktop"><i></i><span></span><span></span><span></span></div><div class="vp-device vp-device--tablet"><i></i><span></span><span></span></div><div class="vp-device vp-device--mobile"><i></i><span></span><span></span></div></div>`,
  "top-nav": ({ copy }) => `
  ...
  sidebar: ({ copy }) => `
    <div class="vp-sidebar-demo"><aside class="vp-side-nav"><span class="vp-side-brand"><i></i>${copy.workspace}</span><small>${copy.mainMenu}</small><div class="vp-side-links"><span class="is-active"><i>⌂</i>${copy.overview}</span><span><i>□</i>${copy.projects}</span><span><i>◇</i>${copy.analytics}</span><span><i>○</i>${copy.team}</span></div><div class="vp-side-user"><i></i><span>${copy.account}<small>${copy.online}</small></span></div></aside><section class="vp-side-page"><div><small>${copy.dashboard}</small><span>•••</span></div>${line("title")}${line("medium")}<div class="vp-side-panels"><i></i><i></i></div></section></div>`,
  breadcrumbs: ({ copy }) => `
    <div class="vp-breadcrumbs-demo"><div class="vp-crumbs"><span>⌂</span><i>›</i><span>${copy.library}</span><i>›</i><span>${copy.collections}</span><i>›</i><b>${copy.summerEdit}</b></div><div class="vp-crumb-page"><small>${copy.collection}</small><strong>${copy.summerEdit}</strong><p>${copy.breadcrumbHint}</p><div>${line("long")}${line("medium")}</div></div></div>`,
  "bottom-tabs": ({ copy }) => `
    <div class="vp-phone"><div class="vp-phone-head">${copy.today}<span class="vp-avatar"></span></div><div class="vp-phone-content">${line("title")}${line("long")}<div class="vp-phone-card"></div></div><div class="vp-bottom-tabs"><span class="is-active"><i>⌂</i>${copy.home}</span><span><i>◇</i>${copy.explore}</span><span><i>♡</i>${copy.saved}</span><span><i>○</i>${copy.profile}</span></div></div>`,
  tabs: ({ copy }) => `
  ...
  "filter-chips": ({ copy }) => `
    <div class="vp-filter-demo"><div class="vp-filter-row">${pill(copy.all, true)}${pill(copy.design)}${pill(copy.product)}${pill(copy.research)}</div><div class="vp-filter-count">30 ${copy.results}</div><div class="vp-filter-items"><span></span><span></span><span></span></div></div>`,
  card: ({ copy, imageUrl }) => `
  ...
  "card-grid": ({ copy, imageUrl }) => `
    <div class="vp-card-grid">${[copy.design, copy.product, copy.research].map((label, index) => `<div class="vp-mini-card">${media(imageUrl, `crop-${index + 1}`)}<small>${escapeHtml(label)}</small>${line(index === 1 ? "long" : "medium")}</div>`).join("")}</div>`,
  list: ({ copy, imageUrl }) => `
    <div class="vp-list">${[copy.listOne, copy.listTwo, copy.listThree].map((label, index) => `<div class="vp-list-row">${media(imageUrl, `thumb-${index + 1}`)}<div><strong>${escapeHtml(label)}</strong>${line("long")}<small>${index + 2}h · ${escapeHtml(copy.read)}</small></div><b>•••</b></div>`).join("")}</div>`,
  "media-tile": ({ copy, imageUrl }) => `
    <div class="vp-media-grid"><figure class="is-large">${media(imageUrl, "tile-1")}<figcaption>${copy.collection}<b>12</b></figcaption></figure><figure>${media(imageUrl, "tile-2")}<figcaption>${copy.studio}</figcaption></figure><figure>${media(imageUrl, "tile-3")}<figcaption>${copy.objects}</figcaption></figure></div>`,
  ...
  drawer: ({ copy }) => `
    <div class="vp-drawer-scene"><div class="vp-drawer-page">${line("title")}${line("long")}<div></div></div><aside class="vp-drawer"><span class="vp-drawer-handle"></span><small>${copy.details}</small><strong>${copy.drawerTitle}</strong>${line("long")}${line("medium")}<dl><div><dt>${copy.owner}</dt><dd>Alex</dd></div><div><dt>${copy.status}</dt><dd>${copy.active}</dd></div></dl></aside></div>`,
  toast: ({ copy }) => `
    <div class="vp-toast-scene"><div class="vp-toast-page"><div></div><div></div><div></div></div><div class="vp-toast"><span>✓</span><div><strong>${copy.savedTitle}</strong><small>${copy.savedBody}</small></div><b>×</b></div></div>`,
  skeleton: ({ copy }) => `
...
export function vocabularyPreviewMarkup(entryOrId, { imageUrl = DEFAULT_IMAGE_URL, language = "zh" } = {}) {
  ...
  return `<div class="vocabulary-preview vocabulary-preview--${id}" data-vocabulary-preview="${id}" aria-hidden="true"><div class="vp-canvas">${factory({ copy, imageUrl: externalImageUrl(imageUrl) })}</div></div>`;
}
