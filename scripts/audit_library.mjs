...
<<<<<<< HEAD
    ...
    try {
      ...
      if (!actual) {
        ...
        requireContract(Math.abs(ratio - targetRatio) <= 0.002, `${item.id}: video file ratio is ${actual.width}:${actual.height}; expected about 9:19.5`);
        if (![[886, 1920], [1080, 2340]].some(([width, height]) => actual.width === width && actual.height === height)) {
...
requireContract((htmlSource.match(/<dialog\b/g) || []).length === 2, "library must expose preview and information dialogs");
requireContract((htmlSource.match(/aria-labelledby=/g) || []).length >= 2, "dialogs are missing accessible labels");
requireContract(/event\.target === previewDialog/.test(jsSource) && /event\.target === infoDialog/.test(jsSource), "dialogs do not close from backdrop clicks");
...
requireContract(/\.phone-screen\s*\{[^}]*overflow:\s*hidden[^}]*isolation:\s*isolate/.test(cssSource), "phone screens do not guarantee rounded clipping");
requireContract(/\.phone-media\s*\{[^}]*object-fit:\s*cover/.test(cssSource), "phone media does not fill the screen");
