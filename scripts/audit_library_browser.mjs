...
<<<<<<< HEAD
    ...
    await page.waitForFunction(() => {
      ...
      return Boolean(video && !video.hidden && video.readyState >= HTMLMediaElement.HAVE_METADATA);
    }, null, { timeout: 5000 });
    const state = await dialog.evaluate((element) => {
      const video = element.querySelector("#previewDialogVideo");
      const phone = element.querySelector(".phone-frame--detail");
      const screen = element.querySelector(".phone-screen");
      const toolbar = element.querySelector(".preview-media-toolbar");
      const rect = element.getBoundingClientRect();
      const phoneRect = phone.getBoundingClientRect();
      const toolbarRect = toolbar.getBoundingClientRect();
      return {
        open: element.open,
        title: element.querySelector("#previewDialogTitle")?.textContent || "",
        mediaReady: Boolean(video?.readyState >= HTMLMediaElement.HAVE_METADATA),
