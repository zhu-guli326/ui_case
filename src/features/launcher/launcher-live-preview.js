const STYLE_VERSION = "20260817-simplified-v1";
...
function sectionMarkup() {
  return `
    <header class="preview-lab-head">
      <div class="preview-lab-head-copy"><h2 id="livePreviewTitle">${localized("页面预览", "Page preview")}</h2><p>${localized("这里是唯一的完整页面预览。上面的平台和 Design System 会实时同步。", "This is the single full-page preview. Platform and design-system choices sync here live.")}</p></div>
    ...
    <div class="preview-lab-stage" id="previewLabStage" data-theme="light"><div class="live-preview-canvas"><div class="preview-device live-preview-device" id="livePreviewDevice" data-size="mobile" data-platform="ios" aria-live="polite"><div class="live-preview-empty">${localized("正在同步页面预览…", "Synchronizing page preview…")}</div></div></div></div>`;
}
...
function init() {
  ...
  new MutationObserver(syncSourcePreview).observe(sourceDevice, { childList:true, subtree:true, attributes:true, attributeFilter:['style','data-platform','data-template'] });
  const systemName = q('#previewSystemName');
  if (systemName) new MutationObserver(syncSystem).observe(systemName, { childList:true, characterData:true, subtree:true });
  window.addEventListener('image2:launcherplatformchange', (event) => { setDeviceForPlatform(event.detail?.platform || currentPlatform()); requestAnimationFrame(syncSourcePreview); });
  document.addEventListener('change', (event) => {
    if (event.target.matches('[name="colorTheme"],[name="fontScheme"],#previewPageTemplate,#previewLanguage')) window.setTimeout(syncSystem, 40);
  });
  window.image2I18n?.registerPage?.(() => { languageSelect.value = locale(); window.setTimeout(syncSystem, 40); });
  setTheme('light');
