...
// Launcher has additional live-preview copy and typography controls that follow the global language switch.
if (/\/launcher\.html$/i.test(window.location.pathname)) {
  ...
  const launcherFontEffectsUrl = new URL("../../features/launcher/launcher-font-effects.js?v=20260829-font-effects-v1", source);
  import(launcherI18nUrl.href).catch(() => {});
  import(launcherFontEffectsUrl.href).catch(() => {});
}
