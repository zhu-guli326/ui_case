...
- `references/ui-reference-benchmark/**` unless the task explicitly asks for benchmark/reference research
...
- historical Library `*-fixes`, density-fix, quality-fix, and root compatibility entries
...
- Launcher: only `src/features/launcher/launcher-dna.css` and `launcher-dna.js` are production implementation; read its `README.md` first for architecture rules.
